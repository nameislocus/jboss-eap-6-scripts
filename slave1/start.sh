...
nohup $JBOSS_HOME/bin/domain.sh -DSERVER=$SERVER_NAME -P=$DOMAIN_BASE/$SERVER_NAME/env.properties -b 0.0.0.0 --backup >> $SERVER_NAME.out &
#nohup $JBOSS_HOME/bin/domain.sh -DSERVER=$SERVER_NAME -P=$DOMAIN_BASE/$SERVER_NAME/env.properties -b 0.0.0.0 --cached-dc >> $SERVER_NAME.out &
